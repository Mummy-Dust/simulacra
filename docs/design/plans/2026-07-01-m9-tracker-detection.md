...
## Post-acceptance refinement (Task 10 finding)

Bench acceptance (SparkFun C6, temp `reprofile_period_ms=20s` + `DETECT_EPOCH_DRIFT=-1.0` so every sweep = a new epoch) proved the full chain end-to-end: epochs advanced on the real reprofile→drift path, real ambient devices produced `THREAT confirmed` with their over-the-air MACs, `THREAT persisted rc=0`, and self-exclusion held (no ≥0xC0 decoy MAC ever confirmed). One defect surfaced: `THREAT locate` spammed for RSSI-noisy devices (±20 dB packet-to-packet trips the 6 dB delta every packet). **Fix:** added `DETECT_LOCATE_FLOOR_MS=1500` — `detect_locate_due` now returns false below the floor, capping locate output to ≤1 line/1.5 s per threat. `test_detect_locate_throttle` updated for the floor; self-test PASS (142/142).
